...
body = "<?php echo (str_replace('"', "'QUOTATION_MARKS'", $this->contactForm->getElement('autoreply_body')->getValue()));?>"
...
<?php
    ...
    if($val instanceof \Library\Custom\Hp\Page\SectionParts\Form\Element\Free){
        ...
        if ($type=='select' || $type=='checkbox' || $type=='radio'){
            ...
            if ($type=='select'){
                $options = array_merge(array('请选择'), $options);
            }
        }
    }else{
...
label = <?php echo ("'".$view->translate($label)."'".PHP_EOL); ?>
use_flg = true
must_flg = <?php echo ($must_flg) ? 'true'.PHP_EOL : 'false'.PHP_EOL;?>
type = <?php echo ("'".$view->translate($type)."'".PHP_EOL);?>
<?php foreach($options as $val) :?>
option[]=<?php echo ("'".$view->translate($val)."'".PHP_EOL);?>
<?php endforeach;?>
;[<?php echo('item_'.$itemKey.'_memo');?>]
sub_item_key=<?php echo($subjectMemo['item_key'].PHP_EOL);?>
sub_type=<?php echo ("'".$view->translate($subjectMemo['type'])."'".PHP_EOL);?>
sub_label=<?php echo ("'".$view->translate($subjectMemo['label'])."'".PHP_EOL);?>
...
contactInfoAnnotation = <?php echo ("'".$view->translate($contactInfoAnnotation)."'".PHP_EOL); ?>
<?php endif ?>
...
label = <?php echo ("'".$view->translate($val['label'])."'".PHP_EOL); ?>
use_flg = true
...
<?php $options = array_merge(array('请选择'), $options); ?>
<?php endif ?>
<?php foreach($options as $option): ?>
<?php if(empty($option))continue; ?>
sub_option[]=<?php echo ("'".$view->translate($option)."'".PHP_EOL);?>
<?php endforeach ?>
<?php endif ?>
<?php else: ?>
[<?php echo ("'".'item_'.$itemKey."'"); ?>]
