...
var dataStatisticsPref = function(params, type, gData) {
    ...
    if (type == OWNERSHIP_TYPE) {
        return getDataOwnerShip(params, type, gData, 1);
    }
...
var dataStatisticsTown = function(params, type, gData) {
    if (type == OWNERSHIP_TYPE) {
        return getDataOwnerShip(params, type, gData, 0);
    }
    var data = [];
...
var getDataOwnerShip = function(params, type, gData, isPref) {
    var data = [0, 0];
    var arrUrlApi = townChartApi[type - 1][isPref];
    params = chunkArray(params, 200);
...
var elavationApi = function(points, distance, gData) {
    ...
    for (var i = 0; i < points.length; i++) {
        ...
        var params = {lat: points[i].lat, lon: points[i].lng, type: 1, position: position};
        api(url, params, gData, function(res) {
            if (i == 0) {
