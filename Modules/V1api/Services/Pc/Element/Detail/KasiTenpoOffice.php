...
class KasiTenpoOffice extends DetailAbstract
{
    ...
    public function createElement(PageInitialSettings $pageInitialSettings, $contentElem, $bukken, $shumoku, Params $params, $codeList, $searchCond)
    {
        ...
        if( !$isKakakuHikokai ) {
            ...
            $priceTxt = $this->getVal('csite_kakaku', $dispModel);
        }
        ...
        if( !$isKakakuHikokai ) {
            $trElem['td:first']->text($this->getVal('csite_shikikin', $dispModel)
                . '/' . $this->getVal('csite_hoshokin', $dispModel));
        }else{
            $trElem['td:first']->text($this->getVal('csite_shikikin', $dispModel));
        }

        //・礼金
        $trElem['td:last']->text($this->getVal('csite_reikin', $dispModel));
        ...
        $tuboTxt = $this->getVal('tatemono_tsubo_su', $dispModel) . '／' . $this->getVal('csite_tsubo_tanka_manen', $dispModel);
        $trElem['td:eq(2)']->text($tuboTxt);

        // 設備タグ
        $this->createSetsubiIcon($shumoku, $mainInfoElem, $dispModel);
    ...
    private function createSetsubiIcon($shumoku, $mainInfoElem, $dispModel)
    {
        ...
        if ($shumoku == Services\ServiceUtils::TYPE_KASI_TENPO) {
    		...
    		$tagListUl = $mainInfoElem['div.article-tag ul']->empty();
    		$tagListUl->append('<li>1階</li>');
    		$tagListUl->append('<li>2階以上</li>');
    		$tagListUl->append('<li>駐車場（近隣含む）</li>');
    		$tagListUl->append('<li>エアコン</li>');
    		$tagListUl->append('<li>男女別トイレ</li>');
    		$tagListUl->append('<li>エレベーター</li>');
            $tagListUl->append('<li>光ファイバー</li>');
    		$tagListUl->append('<li>即引渡し可</li>');
    		$tagListUl->append('<li>24時間利用可</li>');
    		$tagListUl->append('<li>OAフロア</li>');
    ...
    protected function createBukkenTab($bukken, $itemElem, $pageInitialSettings, $shumoku, $codeList, $fdp)
    {
        ...
        if( !$isKakakuHikokai ) {
            $trElem['td:first']->text($this->getVal('csite_shikikin', $dispModel) . '/' . $this->getVal('csite_hoshokin', $dispModel));
        }else{
            $trElem['td:first']->text($this->getVal('csite_shikikin', $dispModel));
        }
        ...
        $shikibiki = $this->getVal('csite_shikibiki', $dispModel);
        $trElem['td:first']->text($shikibiki);
        $hoshokin_shokyaku = $this->getVal('csite_hoshokin_shokyaku', $dispModel);
        ...
        $trElem['td:last']->text($this->getVal('csite_tsubo_tanka_manen', $dispModel));
        // クレジットカード決済
        ...
        $trElem['td:first']->text($this->getVal('csite_koshin_ryo', $dispModel));
