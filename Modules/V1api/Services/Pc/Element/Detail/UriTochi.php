...
class UriTochi extends DetailAbstract
{
    ...
    public function createElement(PageInitialSettings $pageInitialSettings, $contentElem, $bukken, $shumoku, Params $params, $codeList, $searchCond)
    {
        ...
        $trElem['td:eq(1)']->text($this->getVal('saiteki_yoto_nm', $dataModel));
        ...
        $trElem['td:eq(1)']->text($this->getVal('tochi_tsubo_su', $dispModel));
        $trElem['td:eq(2)']->text($this->getVal('tsubo_tanka_manen', $dispModel));
        // 私道負担面積　引渡条件　建ぺい率／容積率
        $trElem = $mainTableElem['tr:eq(4)'];
        $trElem['td:eq(0)']->text($this->getVal('csite_shido_futan_ms', $dispModel));
        $trElem['td:eq(1)']->text($this->getHikiwatashi($dataModel));
        ...
        if (! in_array("08023", $kodawari)) {
            $tagList->eq(0)->addClass('off');
        } // 建築条件なし
        if (! in_array("08024", $kodawari)) {
    ...
    protected function createBukkenTab($bukken, $itemElem, $pageInitialSettings, $shumoku, $codeList, $fdp)
    {
        ...
        // 国土法届出　セットバック
        $trElem = $detailTableElem4['tr:eq(6)'];
        ...
        //$trElem['td:first']->text($this->getVal('er_jokento', $dispModel));
        $remarks = $detailElem['.building_conditions'];
        $detailElem['.building_conditions']->remove();
        $jokento = [];
