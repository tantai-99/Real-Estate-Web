...
class History extends Services\AbstractElementService
{
	public $head;
	public $header;
    public $content;
			...
			Datas $datas)
	{
		...
		$head->title = "最近見た物件リスト｜${siteName}";
		$head->keywords = "最近見た物件,${keyword}";
		$head->description = "最近見た物件リストページ。${description}";
	...
	private function header(
			...
			Datas $datas)
	{
		return "<h1 class='tx-explain'>最近見た物件一覧</h1>";
	}
			...
			Datas $datas)
	{
		...
		$doc = $this->getTemplateDoc("/history/content.sp.tpl");
        ...
        $wrapperElemRent = $doc['div.list-history.rent'];
        $wrapperElemParking = $doc['div.list-history.parking'];
        $wrapperElemOffice = $doc['div.list-history.office'];
        $wrapperElemOthers = $doc['div.list-history.others'];
        $wrapperElemMansion = $doc['div.list-history.mansion'];
        $wrapperElemHouse = $doc['div.list-history.house'];
        $wrapperElemLand = $doc['div.list-history.land'];
        $wrapperElemBusiness = $doc['div.list-history.business'];
        ...
        if($sort_support) {
            ...
            if(is_null($searchTab) || empty($searchTab)) {
                ...
                foreach ($doc['div.element-search-tab4.' . $checklistTab . ' li'] as $searchLiDom) {
                    ...
                    if(!in_array($searchTabTmp, array_keys($cnt))) {	// activeの可能性がある
            ...
            $doc['div.list-history']->attr('style', 'display:none');
            $doc['div.list-history.' . $searchTab]->attr('style', 'display:block');

            $sortOptions = [];
            $sortOptions[] = ['value' => 'asc', 'name' => '閲覧順' ];
