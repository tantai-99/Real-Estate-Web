...
class SplCondition extends Services\AbstractElementService
{
	public $head;
	public $header;
	public $content;
...
// 1. ホーム＞種別選択：1＞(都道府県選択：4＞)沿線から探す：6＞{$沿線名：９}＞{$沿線名}の{$物件種目}一覧
            case $params::SEARCH_TYPE_LINE:
                $levels += array("/{$specialRow->filename}/${ken_ct}/line.html" => '沿線から探す');
                $levels += array("/{$specialRow->filename}/${ken_ct}/${ensen_ct}-line/" => "${ensen_nm}");
                $levels += array("/{$specialRow->filename}/${ken_ct}/result/${ensen_ct}-line.html" => "${ensen_nm}の物件一覧");
                break;
// 2. ホーム＞種別選択：1＞(都道府県選択：4＞){$都道府県名}：5＞{$市区名}の{$物件種目}一覧
            case $params::SEARCH_TYPE_CITY:
                $levels += array("/{$specialRow->filename}/${ken_ct}/" => "${ken_nm}");
                $levels += array("/{$specialRow->filename}/${ken_ct}/result/${shikugun_ct}-city.html" => "${shikugun_nm}の物件一覧");
                break;
// 3. ホーム＞種別選択：1＞(都道府県選択：4＞){$都道府県名}：5＞{$政令指定都市}の{$物件種目}一覧
            case $params::SEARCH_TYPE_SEIREI:
                $levels += array("/{$specialRow->filename}/${ken_ct}/" => "${ken_nm}");
                $levels += array("/{$specialRow->filename}/${ken_ct}/result/${locate_ct}-mcity.html" => "${locate_nm}の物件一覧");
                break;
// 4. ホーム＞種別選択：1＞(都道府県選択：4＞)沿線から探す：6＞{$沿線名：９}＞{$駅名}の{$物件種目}一覧
            case $params::SEARCH_TYPE_EKI:
                $levels += array("/{$specialRow->filename}/${ken_ct}/line.html" => '沿線から探す');
                $levels += array("/{$specialRow->filename}/${ken_ct}/${ensen_ct}-line/" => "${ensen_nm}");
                $levels += array("/{$specialRow->filename}/${ken_ct}/result/${eki_ct}-eki.html" => "${eki_nm}駅の物件一覧");
                break;
// 5. ホーム＞種別選択：1＞(都道府県選択：4＞)地図から探す：7＞{$市区名}の{$物件種目}地図一覧
//  next
// 6. ホーム＞種別選択：1＞(都道府県選択：4＞){$都道府県名}の{$物件種目}一覧
            case $params::SEARCH_TYPE_PREF:
                $levels += array("/{$specialRow->filename}/${ken_ct}/result/" => "${ken_nm}の物件一覧");
                break;
...
// 8. ホーム＞種別選択：1＞(都道府県選択：4＞){$都道府県名}：5＞{$都道府県名}の{$物件種目}一覧
         ...
         * 見出し処理
         */
        $doc['h2']->text("{$specialRow->title}の条件を絞り込む");
            ...
            case $params::SEARCH_TYPE_CHOSON_POST:
                if ($specialSetting->area_search_filter->canChosonSearch()) {
                    if (count($pNames->getShikuguns()) > 5) {
                        $url = "/{$specialRow->filename}/{$ken_ct}/";
                        ...
                        $url = "/{$specialRow->filename}/{$ken_ct}/city/search/";
        ...
        $kodawari = $searchFilterElement->createParticularTableElement();
        if(count($kodawari->find('dd'))) {
           $doc['section.element-narrow-down']->append( $kodawari );
        }
