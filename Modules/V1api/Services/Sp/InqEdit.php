...
class InqEdit extends Services\AbstractElementService
{
			...
			Datas $datas)
	{
		...
		$doc = $this->getTemplateDoc("/inquiry/content.sp.tpl");
        ...
        foreach ($bukkenList['bukkens'] as $bukken)
        {
            ...
            $bukkenElem = $bukkenMaker->createElementHachi($shumoku, $dispModel, $dataModel, $params, true, $dispModel->niji_kokoku_jido_kokai_fl, $settings->page, $settings->search);
            // 詳細画面には飛ばさないため、アンカーをつぶす
            $bukkenElem['.object-body a:first']->after($bukkenElem['.object-body a:first']->html())->remove();

            $doc['div.list-inquery']->append($bukkenElem);
        }
