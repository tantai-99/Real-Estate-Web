...
class Koma
{
    ...
    public function createKoma($komaElem, $komaList, $params, $pageInitialSettings)
    {
        ...
        // .element-recommend
        $komaElem->append(pq('<div class="element element-recommend-caro estate-koma" />'));
        $komaElem['div.element-recommend-caro']->attr('data-special-path', $params->getSpecialPath());
        ...
        for ($i=0; $i < $row * 4 && $i < $total; $i++)
        {
            ...
            //   <a href="#">
            //     <p class="recommend-caro-ph"><img src="imgs/ph_recommend1.jpg" alt=""></p>
            //     <div class="recommend-caro-info">
            //       <p class="recommend-caro-name">1物件名ハイリマステキスト長いバージョン</p>
            //       <p class="tx-price">〇〇万円</p>
            //       <p class="recommend-caro-kind">2DK/アパート</p>
            //       <p class="recommend-caro-station">JR山手線「新宿駅」徒歩10分</p>
            //     </div>
            //   </a>
            // </div>
            ...
            $divElem = pq('<div class="recommend-caro-item" />');
            // image
            $thumbnail = Services\ServiceUtils::getMainImageKoma($dataModel, $params);
            $img_url = '';
            $img_domain = $this->getImgDomain($img_server);
            if (!is_null($thumbnail)) {
                $img_url = $img_domain.$thumbnail->url."?width=138&amp;height=138&amp;margin=true";
            }
            else {
                $img_url = $img_domain."/image_files/path/no_image";
            }

            $aElem = pq('<a href="' . $detail_url . '" target="_blank"></a>');
            $aElem->append('<p class="recommend-caro-ph"><img  src="'. $img_url .'" data-original="'. $img_url .  '" alt=""></p>');
            $divInfoElem = pq('<div class="recommend-caro-info" />');
            $divInfoElem->append('<p class="recommend-caro-name">'. $this->getVal('csite_bukken_title', $dispModel, true) . '</p>');
            if(!$isKakakuHikokai) {
                $divInfoElem->append('<p class="tx-price">' . Services\ServiceUtils::replaceSsiteBukkenTitle($this->getVal('csite_bukken_title', $dispModel, true)) .'</p>');
            }else{
                $divInfoElem->append('<p class="tx-price">' . "賃料要確認" .'</p>');
            }
            $divInfoElem->append('<p class="recommend-caro-kind">' . $this->getMadoriMenseki($dispModel) . '/' . Services\ServiceUtils::getShumokuDispModel($dispModel) . '</p>');
            $divInfoElem->append('<p class="recommend-caro-station">' . $this->csiteKotsuSubstr($dispModel) . '</p>');
            $aElem->append($divInfoElem);
            $divElem->append($aElem);
            $komaElem['div.element-recommend-caro']->append($divElem);
        }
    }
