			...
			<div class="item-set-list2 sub-elements sortable-item-container">
				...
				<div class="item sortable-item added-item" data-name="<?php echo $key2?>" data-type="<?php echo $element->getType()?>" data-is-preset="<?php echo $element->isPreset()?>" data-title="<?php echo $element->getTitle()?>" data-is-unique="<?php echo $element->isUnique()?>">
					...
					<dl class="item-set-header">
						<dt><span>Q</span></dt>
						<dd>
							<?php echo $element->simpleText('q')?><span class="input-count"></span>
							<div class="errors"></div>
						</dd>
						<dd>
							<select class="qa-sample w150">
								<option value="">雛形選択</option>
								<?php foreach ($element->getSamples() as $idx => $sample):?>
								<option value="<?php echo $idx?>" data-a="<?php echo h($sample->a)?>"><?php echo $sample->q?></option>
								<?php endforeach;?>
							</select>
						</dd>
						<dd class="action">
					...
					<dl>
						...
						<dd class="element-text-utilcontainer element-text">
							<div class="mb20">
							<?php $element->simpleText('a')?><span class="input-count"></span>
							<div class="errors"></div>
							</div>
				...
				<div class="item-add">
					<a href="javascript:void(0);" class="btn-t-blue size-s">Q&Aを追加</a>
				</div>
