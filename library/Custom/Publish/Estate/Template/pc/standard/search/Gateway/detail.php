...
class Detail extends _AbstractGateway {
    ...
    public function run() {
        ...
        // おすすめ物件からのアクセス
        if ($this->request->getPost('from-recommend') === 'true') {
            $url .= $this->addParam(ApiGateway::KEY_FROM_RECOMMEND, 'true');
        };
...
//             return $view;
        ...
        // p.photo-zoomがcontentに含まれているのでhiddenに移動
        ...
        if ($this->request->getPost('from-recommend') === 'true') {
            ...
            $configObject->from_recommend = true;
