...
<?php foreach ($parts->getSubForms() as $page_id => $item): ?>
  <div class="element element-colum-list">
    <a href="<?php echo $view->hpLink($item->getPage()->link_id) ?>">
      <div class="element-colum-list-left"><?php if ($item->getValue('image')): ?><img src="<?php echo $view->hpImage($item->getValue('image')) ?>" alt="<?php echo $item->getValue('image_title') ?>"/><?php endif ?>
      </div>
