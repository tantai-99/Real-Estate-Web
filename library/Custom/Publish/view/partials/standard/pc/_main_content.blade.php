...
if ($mainImage = $view->page->form->getSubForm('main_image')) {
    ...
    if (isset($imageConfig->slide_show_flg)) {
        ...
        switch ($imageConfig->type_slideshow) {
            ...
            //     $configSlick->centerPadding = '70px';
            //     break;
        }
...
<?php if ($hpPageTable->isEstateContactPageType($view->page->getRow()->page_type_code)): ?>
    <section>
        <?php echo $view->partial('main-parts/heading.blade.php',
            array('heading' => "物件のお問い合わせ",'level' => 0, 'isEstateContact' => true)) ?>

<?php // elseif( in_array($view->hp->theme_id, array(21,22,23))) : ?>
<?php elseif( in_array($view->theme->name, array('standard02_custom_color','natural02_custom_color','simple02_custom_color')) ) : ?>
    <section>
    <?php if ($hpPageTable->isEstateRequestPageType($view->page->getRow()->page_type_code)): ?>
        <?php echo $view->partial('main-parts/heading.blade.php',
            array('heading' => $view->page->form->getSubForm('tdk')->getValue('title'),'level' => 0, 'isEstateContact' => true)) ?>
    <?php elseif ($view->page->form->getSubForm('tdk')): ?>
        <?php echo $view->partial('main-parts/heading.blade.php',
            array('heading' => $view->page->form->getSubForm('tdk')->getValue('title'),
                  'level' => 0, 'isEstateContact' => true, 'hp' => $view->hp)
            ) ?>
    <?php endif; ?>
...
<?php elseif ($view->page->form->getSubForm('tdk')): ?>
    <section>
        <?php echo $view->partial('main-parts/heading.blade.php',
            array('heading' => $view->page->form->getSubForm('tdk')->getValue('title'),'level' => 0, 'hp' => $view->hp)) ?>
<?php endif; ?>
...
<?php if ($view->page->form->getSubForm('main')) : ?>
    ...
    <?php if($view->page->getRow()->page_type_code == HpPageRepository::TYPE_INFO_DETAIL) :?>
        ...
        ?>
        <div class="element<?php echo $view->isTopOriginal ? ' element-top-news' : ''?>"<?php echo isset($hpMainPart) ? ' data-category="'.$hpMainPart->attr_2.'"' : ''?><?php echo isset($hpMainPart) ? ' data-category-class="'.$hpMainPart->attr_3.'"' : ''?>>
            <?php $date = date('Y-m-d', strtotime(str_replace(array('年', '月', '日'), array('-', '-', ''), $view->page->form->getSubForm('tdk')->getValue('date'))));?>
            <?php if ($view->isPreview) :?>
            <?php
            $pageIndex = $hpPageTable->find($view->page->getRow()->parent_page_id);
    ...
    <?php foreach ($view->page->form->getSubForm('main')->getSubForms() as $area) : ?>
        ...
        <?php foreach ($area->getPartsByColumn() as $column): ?>
            ...
            <?php foreach ($column as $parts): ?>
                ...
                <?php elseif ($parts instanceof \Library\Custom\Hp\Page\Parts\ColumnDetail): ?>
                    <?php echo $view->partial('main-parts/column-detail.blade.php', array('element' => $parts, 'area' => $area, 'page' => $view->page, 'all_pages' => $view->all_pages)); ?>

                <?php elseif ($parts instanceof \Library\Custom\Hp\Page\Parts\CompanyOutline): ?>
                    <?php echo $view->partial('main-parts/company.blade.php', array('element' => $parts, 'area' => $area, 'page' => $view->page, 'all_pages' => $view->all_pages, 'isEstateContact' => null)); ?>
                ...
                <?php
                    ...
                    <?php //echo $parts->getTemplate('main-parts/');?>
                    <?php echo $view->partial($parts->getTemplate('main-parts/'),
                                array('element' => $parts, 'area' => $area, 'page' => $view->page, 'all_pages' => $view->all_pages, 'hp' => $view->hp, 'inside_division' => false, 'heading' => null , 'isEstateContact' => false)); ?>
                <?php endif ?>
            <?php endforeach; ?>
...
<?php if ($view->page->form->getSubForm('list')): ?>
    ...
    echo $view->partial('main-parts/' . $template, array('pages' => $view->page_list, 'listNumber' => $view->listNumber, 'listCount' => $view->listCount, 'page' => $view->page, 'blog_yyyymm' => $view->blog_yyyymm, 'isEstateContact' => null));
    ?>
<?php endif ?>

<?php if ($hpPageTable->isEstateContactPageType($view->page->getRow()->page_type_code) || $view->page->form->getSubForm('tdk')): ?>
</section>
<?php endif; ?>
