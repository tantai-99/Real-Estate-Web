...
?>
<section>
    <h2 class="heading-lv1"><span><?php if ($view->is404) : ?>ページが見つかりません<?php else : ?>サイトマップ<?php endif; ?></span></h2>
    ...
    <section>
        ...
        <div class="element">
            <ul class="link-pagelist">
                ...
                <?php endforeach; ?>
                <?php if (count($pagesInfo) > 0) :?>
                    <?php foreach ($pagesInfo as $page): ?>
                    <li><a <?= $view->hpHref($page); ?>><?php echo h($page['title']); ?></a></li>
                    <?php endforeach; ?>
                <?php endif; ?>
                <?php foreach ($view->contentsArticle as $page): ?>
                    <li><a <?= $view->hpHref($page); ?>><?php echo h($page['title']); ?></a></li>
                <?php endforeach; ?>
            </ul>
        </div>
