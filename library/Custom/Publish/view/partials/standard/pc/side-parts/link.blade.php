...
$link = true;
foreach ($view->element->elements->getElements() as $element) {
    $company = \App::make(CompanyRepositoryInterface::class)->fetchRowByHpId($view->element->getHp()->id);
        if ($company->cms_plan == config('constants.cms_plan.CMS_PLAN_LITE')) {
            if ($view->element->getValue('link_type') == config('constants.link_type.PAGE')) {
                if (!is_numeric($view->element->getValue('link_page_id'))) {
                    $link = false;
                    break;
                }
            }
    ...
    <?php foreach ($view->element->elements->getSubForms() as $element): ?>
      <li>
            ...
            <?php foreach ($view->all_pages as $page) : ?>
              <?php if ($page['link_id'] == $element->getValue('link_page_id')) : ?>
                <?php if (in_array($page['page_type_code'], \App::make(HpPageRepositoryInterface::class)->getCategoryMap()[HpPageRepository::CATEGORY_FORM])) : ?>
                  <a href="<?php echo $view->hpLink($element->getValue('link_page_id')) ?>" <?php if ($element->getValue('link_target_blank')) : ?>target="_blank"<?php endif; ?>><?php echo h($page['title']) ?></a>
        ...
        <?php elseif ( $element->getValue( 'link_type' ) == config('constants.link_type.URL') ): ?>
          <?php // 外部リンク ?>
          <a href="<?php echo h($element->getValue('link_url')) ?>" <?php if ($element->getValue('link_target_blank')) : ?>target="_blank"<?php endif; ?>><?php echo h($element->getValue('link_label')) ?></a>
