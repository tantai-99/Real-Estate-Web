...
class sidenav {
    ...
    public function __construct($viewObj, $device, $publishType, $sidebarOtherLinkTitle, $filename = null, $pages = null, $thisPage = null) {
        ...
        if ($device == 'sp') {
            ...
            // 非表示
            // - 下層ページない
            if (count($this->getChildrenPages($this->firstLevelPage['id'], $pages)) < 1) {
                $this->display = false;
            }
            ...
            // お知らせ詳細
            // 会員専用ページ
            switch ($thisPage['page_type_code']) {
                case $TYPE_INFO_INDEX = 2 :
                case $TYPE_INFO_DETAIL = 3 :
                case $TYPE_MEMBERONLY = 48 :
                    $this->display = false;
                    break;
                default:
                    break;
    ...
    public function echoChild($parentPageId, $pages) {
        ...
        // HpPageRepositoryNFO_INDEX = 2;
        // HpPageRepositoryLOG_INDEX = 14;
        $TYPE_BLOG_INDEX = 14;
        $TYPE_MEMBERONLY = 48;
