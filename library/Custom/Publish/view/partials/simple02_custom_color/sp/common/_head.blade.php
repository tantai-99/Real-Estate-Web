...
<?php
    ...
    if($view->isTop && $view->company->cms_plan < config('constants.cms_plan.CMS_PLAN_ADVANCE')):?>
    <link rel="stylesheet" href="<?php $view->src('css/slick-custom.css') ?>"/>
<?php endif; ?>
...
?>
    ...
    <?php if ($view->isPreview): ?>
        ...
        <style>
        @font-face { 
            font-family: 'athm-spl2'; 
            src: url("<?php $view->src('fonts/athm-spl2.ttf'); ?>") format("truetype"), 
            url("<?php $view->src('fonts/athm-spl2.woff'); ?>") format("woff"), 
            url("<?php $view->src('fonts/athm-spl2.svg'); ?>") format("svg"); 
            font-weight: normal; font-style: normal;
        }
        </style>
    <?php endif; ?>
	<?php if($view->isTop):
        if($view->company->cms_plan < config('constants.cms_plan.CMS_PLAN_ADVANCE') ){
           $view->configSlick->arrows=true;
        }
    ?>
    <script> 
