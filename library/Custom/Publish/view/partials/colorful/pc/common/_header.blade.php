...
<header class="page-header" role="banner">
  <div class="page-header-top">
    <div class="inner">
      ...
      <?php endif; ?>
      <ul class="link">
        <li><a href="<?= $siteUrl; ?>/sitemap/">サイトマップ</a></li>
        <li><a <?= $view->hpHref($view->pageContact); ?> target="_blank">お問い合わせ</a></li>
      </ul>
      <?php if (count($view->getPublishEstateInstance()->estateTypes) > 0): ?>
        <ul class="link2">
          <li class="link2-fav"><a href="<?= $siteUrl; ?>/personal/favorite/">お気に入り物件</a></li>
          <li class="link2-history"><a href="<?= $siteUrl; ?>/personal/history/">最近見た物件</a></li>
        </ul>
      <?php endif; ?>
    </div>
  </div>
  <div class="inner">
    <p class="logo">
      <a href="<?= $siteUrl; ?>">
        <?php if (h($view->hp->logo_pc)) : ?>
          <span class="company-img"><img src="<?php if ($view->isPreview): ?>/image/site-logo-pc?id=<?php echo $view->hp->logo_pc ?><?php else : ?>/images/logo_pc.<?= $view->siteImagePc->extension; ?><?php endif; ?>" alt="<?= h($view->hp->logo_pc_title); ?>"></span>
        <?php endif; ?>
        <?php if (h($view->hp->logo_pc_text)) : ?>
          <span class="company-tx"><?= h($view->hp->logo_pc_text); ?></span>
        <?php endif; ?>
      </a>
    </p>
    <?php if ($view->hp->tel || $view->hp->office_hour) : ?>
      <div class="header-info">
        <?php if ($view->hp->tel) : ?>
          <p class="tel"><?= h($view->hp->tel); ?></p>
        <?php endif; ?>
        <?php if ($view->hp->office_hour) : ?>
          <p class="time">営業時間/<?= h($view->hp->office_hour); ?></p>
        <?php endif; ?>
      </div>
    <?php endif; ?>
    <?php if ($view->hp->fb_like_button_flg || $view->hp->tw_tweet_button_flg) : ?>
      <div class="header-sns">
        <?php if ($view->hp->fb_like_button_flg) : ?><?= $view->partial('common/_fblike.blade.php'); ?><?php endif; ?>
        <?php if ($view->hp->tw_tweet_button_flg) : ?><?= $view->partial('common/_tweetbtn.blade.php'); ?><?php endif; ?>
      </div>
    <?php endif; ?>
  </div>
</header>
